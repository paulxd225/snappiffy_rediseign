  ...
  return (
    <div className="flex flex-col gap-3 w-full">
      ...
      <p className="text-xs text-neutral-600">
        Scroll with your mouse wheel over the strip to move horizontally.
      </p>
      <div
        ref={scrollRef}
        className={cn(
          'tech-strip-scroll flex w-full gap-3 overflow-x-auto overflow-y-hidden rounded-2xl border border-emerald-200/80 bg-white/80 p-4 shadow-sm',
        )}
      >
        {TECH_STACK.map((name) => (
          <div
            key={name}
            className="shrink-0 rounded-xl border border-emerald-100 bg-gradient-to-br from-emerald-50 to-white px-5 py-3 text-sm font-bold text-[#064e3b]"
          >
            {name}
          </div>
        ))}
...
export function VideoIntroSection() {
  return (
    ...
    >
      <div className="mx-auto flex flex-col max-w-7xl gap-10 px-4 sm:px-6 lg:grid lg:grid-cols-2 lg:items-center lg:gap-14 lg:px-8">
